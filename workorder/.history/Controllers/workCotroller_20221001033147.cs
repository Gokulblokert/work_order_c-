...
[HttpDelete("Remove/workid")]
     ...
     public  IActionResult assigntechnician(int workid,int techid)
    {
        ...
        try
        {
            ...
            var techid1 = this._DBContext.TechnicianTbs.FirstOrDefault(t => t.TId ==  techid && t.TStatus == "1");
 if (techid1 != null)  
            {  

		var workorder = this._DBContext.WorkTbs.FirstOrDefault(w => w.WorkId == workid);
		 if (workorder != null)  
            {   
