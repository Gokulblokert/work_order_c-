...
[HttpDelete("Remove/workid")]
    ...
    [HttpPost("CreateTech")]
     public  IActionResult CreateTech([FromBody] TechnicianTb tecch)
    {
        try
        {
        ...
        [HttpPut("deactivetechnician/id")]
     public  IActionResult deactivetechnician(string tech)
    {
        try
        {

            
            var techh = this._DBContext.TechnicianTbs.FirstOrDefault(t => t.TId == tech);
            if (techh != null)
            {
                techh.TStatus = "0";
                this._DBContext.SaveChanges();
 ...
 if (techid1 != null)  
		...
		var workorder = this._DBContext.WorkTbs.Where(w => w.WorkId == workid).FirstOrDefault();
		 if (workorder != null)  
            {   
 		        workorder.TechicianRegisterno =techid1.TId;  
